...
// Hjälpfunktion för att köra funktioner säkert utan att hela filen dör vid fel
...
function initMessagesPage() {
    ...
    if (readModalEl) {
        readModalEl.addEventListener("show.bs.modal", (event) => {
            const row = event.relatedTarget?.closest('tr.message-row');
            if (!row) return;
            ...
            // --- LOGIK FÖR ATT VISA/DÖLJA SVARA-KNAPPEN ---
            if (replyBtn) {
                // Om senderId saknas eller är "null" (sträng från HTML), betrakta som extern
                const isExternal = !senderId || senderId === "" || senderId === "null";

                if (isExternal) {
                    replyBtn.style.display = 'none';
                } else {
                    // Återställ till synlig för inloggade användare
...
// ==========================================
// --- DOM LOAD (Kör allt säkert) ---
// ==========================================

document.addEventListener("DOMContentLoaded", function () {
    // Säkra anrop till alla moduler
    safeInit(initAddProjectSuccessPopup, "Success Popup");
    ...
    // All Projects Join Buttons (Delegation)
    document.addEventListener("click", (e) => {
        const btn = e.target.closest('.all-projects-join-btn');
        if (btn) {
            const id = btn.getAttribute('data-project-id');
            const name = btn.getAttribute('data-project-name');
            openJoinRoleModal(id, name);
        }
    });
